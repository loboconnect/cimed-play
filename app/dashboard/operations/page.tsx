...
export default function OperationsPage() {
  ...
  const sendCommand = async (commandType: CommandType, payload: any = {}) => {
    ...
    try {
      ...
      // Comando enviado com sucesso - será exibido via realtime
    } catch (err: any) {
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-400 mt-2">
            Operador: {user.email} | Role: {userRole}
          </p>
        </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-gray-900 rounded-lg border border-gray-800 p-6">
            ...
            <div className="space-y-4">
              <button
                onClick={() => sendCommand('START_OPERATION')}
                disabled={loading}
              ...
              <button
                onClick={() => sendCommand('STOP_OPERATION')}
                disabled={loading}
                className="w-full px-4 py-3 bg-yellow-600 hover:bg-yellow-700 disabled:bg-yellow-400 text-white font-semibold rounded-lg transition-colors"
              >
                {loading ? 'Enviando...' : '⏹️ STOP OPERATION'}
              </button>

              <button
                onClick={() => sendCommand('TRIGGER_EFFECT')}
                disabled={loading}
              ...
              <button
                onClick={() => sendCommand('LOCK_SYSTEM')}
                disabled={loading}
                className="w-full px-4 py-3 bg-orange-600 hover:bg-orange-700 disabled:bg-orange-400 text-white font-semibold rounded-lg transition-colors"
              >
